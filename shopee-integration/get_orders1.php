...
$url = BASE_URL . $path . "?" . http_build_query([
    ...
    'page_size' => 50,
    'cursor' => '',
    'order_status' => 'READY_TO_SHIP', // Hanya ambil order dengan status 'READY_TO_SHIP'
...
file_put_contents("orders1.json", $response); // Tambahkan baris ini
echo "<h3>Order List Response (HTTP Status: $http_code):</h3><pre>$response</pre>";

header("Location: get_orders2.php");
exit;

// Cetak respons jika ingin debugging (setelah redirect)
echo "<h3>Order List Response (HTTP Status: $http_code):</h3><pre>$response</pre>";